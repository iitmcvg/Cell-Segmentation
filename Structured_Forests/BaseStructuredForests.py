...
class BaseStructuredForests(object):
    ...
    def get_reg_ftr(self, channels, smp_loc=None):
        ...
        reg_ftr = reg_ftr.reshape((int(n_r - p_size + 1), int(n_c - p_size + 1),
                                   int(p_size ** 2 * n_ch)))

        if smp_loc is not None:
            r_pos = [int(r - p_size / 2) for r, _ in smp_loc]
            c_pos = [int(c - p_size / 2) for _, c in smp_loc]
            # print('r_pos:{}, c_pos:{}'.format(len(r_pos), len(c_pos)))
            # condition = N.zeros_like(reg_ftr, dtype = N.bool)
            # condition[r_pos, c_pos] = True
    ...
    def get_ss_ftr(self, channels, smp_loc=None):
        ...
        if smp_loc is not None:
            ss_ftr = ss_ftr.reshape((int(n_r - p_size + 1), int(n_c - p_size + 1),
                                     int(p_size ** 2), int(n_ch)))
            r_pos = [int(r - p_size / 2) for r, _ in smp_loc]
            c_pos = [int(c - p_size / 2) for _, c in smp_loc]
        ...
        grid_pos = [int(r * p_size + c) for r in grid_pos for c in grid_pos]
